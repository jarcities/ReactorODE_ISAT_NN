...
#define RTOL  SUN_RCONST(1.0e-8) /* scalar relative tolerance            */
#define ATOL1 SUN_RCONST(1.0e-8) /* vector absolute tolerance components */
#define ATOL2 SUN_RCONST(1.0e-8)
...
int main(int argc, char* argv[])
{
  ...
  sunrealtype p1 = data->p[0];
  sunrealtype p2 = data->p[1];
  sunrealtype p3 = data->p[2];

  /* Initialize y */
  Ith(y, 1) = SUN_RCONST(1.0) + p1 - SUN_RCONST(1.0);
  Ith(y, 2) = SUN_RCONST(0.5) + p2 - SUN_RCONST(1.0);
  Ith(y, 3) = SUN_RCONST(0.3) + p3 - SUN_RCONST(1.0);
  ...
  if (sensi)
  {
	...
	sunrealtype *ySdata = N_VGetArrayPointer(yS[0]);	
...
static int f(sunrealtype t, N_Vector y, N_Vector ydot, void* user_data)
{
  ...
  Ith(ydot, 1) = -y1 + 0.1*y2 + 0.03*y3;
  Ith(ydot, 2) = 0.008*y1 - 2*y2 - 0.01*y3;
  Ith(ydot, 3) = -0.013*y1 + 0.017*y2 -3*y3;
               ...
               void* user_data, N_Vector tmp1, N_Vector tmp2, N_Vector tmp3)
{
  ...
  IJth(J, 1, 1) = SUN_RCONST(-1.0);
  IJth(J, 1, 2) = SUN_RCONST(0.1);
  IJth(J, 1, 3) = SUN_RCONST(0.03);
  IJth(J, 2, 1) = SUN_RCONST(0.008);
  IJth(J, 2, 2) = SUN_RCONST(-2.0);
  IJth(J, 2, 3) = SUN_RCONST(-0.01);
  IJth(J, 3, 1) = SUN_RCONST(-0.013);
  IJth(J, 3, 2) = SUN_RCONST(0.017);
  IJth(J, 3, 3) = SUN_RCONST(-3.0);
              ...
              N_Vector tmp2)
{
  ...
  sd1 = -s1 + 0.1*s2 + 0.03*s3;
  sd2 = 0.008*s1 - 2.0*s2 - 0.01*s3;
  sd3 = -0.013*s1 + 0.017*s2 - 3*s3;

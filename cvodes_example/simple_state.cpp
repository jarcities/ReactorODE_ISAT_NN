...
// class to create object to represent the system of odes for the reactor from cantera's Funceval to interface with integrator
class ReactorODEs : public FuncEval
{
public:
    // constructor initializes thermodynamic and kinetic models from the solution object
    ...
    size_t nparams() const override { return m_nSpecies + 1; } //SENSITIVITY

    // return number of senstivities

    // evaluates the time derivatives of the system at time t
    void eval(double t, double *y, double *ydot, double *p) override
    {

        // update the thermodynamic state based on current temperature and mass fractions
    ...
    // provides the ic (initial species mass fractions and intial temp.) for the integrator
    ...
    // get the state of the system at time t
...
int main()
{
    ...
    // set number of parameters for sensitivity analysis
    size_t npar = reactor.nparams();
    std::vector<double> y0(npar);
    reactor.getInitialConditions(0.0, y0.data());
    reactor.m_sens_params    = y0;
    reactor.m_paramScales.assign(npar, 1.0);

    // create and initialize the cvode integrator
    auto integrator = newIntegrator("CVODE");
    integrator->setTolerances(1e-9, 1e-15);

    // set intial conditions for sensitivity equations
    integrator->setSensitivityTolerances(1e-9, 1e-15);

    // initialize intergrator with the reactor object
    ...
    // print results
    std::cout << "Final temperature: " << y[gas->nSpecies()] << " [K]" << std::endl;

    //+++++++++++++++++++++++++++++++++++++++
    size_t nParams = integrator->nSensParams(); // should equal nSpecies+1
    size_t temp_idx = reactor.neq() - 1;        // last state = temperature

    std::cout << "Sensitivities of final T wrt initial conditions:\n";
    for (size_t j = 0; j < nParams; ++j)
    {
        double dTdp = integrator->sensitivity(temp_idx, j);
        if (j < reactor.neq() - 1)
        {
            std::cout << "  dT/dY" << j << "0 = " << dTdp << "\n";
        }
        else
        {
            std::cout << "  dT/dT0  = " << dTdp << "\n";
        }
    }
    //+++++++++++++++++++++++++++++++++++++++
